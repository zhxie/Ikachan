...
class ShiftIntentHandler: IntentHandler, ShiftIntentHandling {
    ...
    func handle(intent: ShiftIntent, completion: @escaping (ShiftIntentResponse) -> Void) {
        ...
        fetchShifts(game: game) { shifts, error in
            ...
            let result = String(format: formatter.localizedIntentsString, game.name.localizedString, shift.rule.name.localizedIntentsString, shift.stage!.name.localizedIntentsString, shift.weapons[0].name.localizedIntentsString, shift.weapons[1].name.localizedIntentsString, shift.weapons[2].name.localizedIntentsString, shift.weapons[3].name.localizedIntentsString, intentsLongTimeSpan(current: Date(), startTime: shift.startTime, endTime: shift.endTime))
            
            let encoder = JSONEncoder()
            let data = try! encoder.encode(shift)
            let activity = NSUserActivity(activityType: String(format: IkachanShiftActivity, game.name))
