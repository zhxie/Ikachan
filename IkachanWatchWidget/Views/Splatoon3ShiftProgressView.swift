...
struct Splatoon3ShiftProgressView : View {
    ...
    var body: some View {
        ...
        default:
            AccessoryCornerView(progress: timePassingBy(current: entry.date, start: entry.shift?.startTime ?? entry.date, end: entry.shift?.endTime ?? entry.date), mode: entry.shift?.mode.accentColor ?? Splatoon3ShiftMode(from: entry.configuration.mode).accentColor, rule: entry.shift?.mode.image ?? nil)
                .widgetContainerBackground(padding: false)
        }
    }
}
